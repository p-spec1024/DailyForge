...
- **Apr 23, 2026 morning** — ✅ **T5a SPIKE PASSED.** `interactive_3d` v2.0.3 verified with our GLB. Mesh names survive export cleanly (format: `chest_L#15` — strip `#<id>` suffix in Dart). Tap detection accurate on a per-mesh basis. iOS verification deferred (no Mac available; will verify at Sprint 11 Google Play / App Store submission time).
- **Apr 23, 2026 morning** — ✅ **T5a Steps 1-3 complete.** Data layer (muscle_groups.dart, mock_body_map_data.dart), heatmap color util, BodyMap3D widget, home page scaffold, and 9 child widgets all written. `flutter analyze` clean. Sprint 8 home preserved at `_legacy/home_page_s8.dart`.
- **Apr 23, 2026 afternoon** — 🐛 **Selection race condition root-caused and fixed.** Initial StatefulWidget design pushed Dart state into native imperatively via `clearSelections()` on every state transition. This raced with the package's own native input pipeline, causing taps to drop after the first and making rotation intermittent. Fix: align with the package's "native owns selection" model, remove reactive clears, keep one explicit clear only on mode switch. Captured as learning in SPRINT_TRACKER.md.
- **Apr 23, 2026 afternoon** — 📋 **T5a architecturally complete; UX polish deferred to post-Blender-resplit.** Issues identified during device testing (bicep split coverage, heatmap not always visible, group-level highlighting not implemented, gesture conflict with scroll view, font fallback) were captured as FUTURE_SCOPE items #93-#102. The Blender re-split for better visual quality (item #93) is the gating item — all downstream visual/UX polish is blocked on it because polishing highlights on meshes being replaced is wasted work.
