...
**Progress:** T1 ✅ → T2 ✅ → T3 ✅ → T3.5 ✅ → **T4 ⏳ (next, branches off `s12-t3.5`)** → T5 → T6 → T7.
...
| 3.5 | State-focus refactor — range-bracket picker + `getAvailableDurations` + `centering/practice/reflection` phase rename | ✅ Shipped Apr 29, 2026 | Architecture pivot driven by T3 smoke (24 DEGRADED cases / 30). Replaces T3's fixed-budget contract with the v2 spec's range-bracket model — picker exposes 5 brackets (`0-10 / 10-20 / 21-30 / 30-45 / endless`). New exported `getAvailableDurations(focus_slug, breathwork_level, userId?)` helper returns per-bracket availability (`available` / `locked_by_level` / `empty`); honors per-user `user_excluded_exercises` when `userId` is provided. State-focus phase tokens renamed `settle/main/integrate` → `centering/practice/reflection` (engine output only — DB column `breathwork_techniques.settle_eligible_for` and internal helper names kept). Endless mode: practice = picked technique's `<level>_duration_max`, with 2-min centering + 2-min reflection. Pick-and-fit retry loop and lowest-min fallback from T3 deleted — pre-filter via `techniqueFitsBracket` guarantees a fit. Body-focus paths, mobility, full_body, recency, swap-counter, HTTP all unchanged. **Pre-flight catch:** live-vs-Appendix-A diagnostic flagged 17 cells of drift in v1 spec; Amendment 1 v1.1 corrects 17 + 2 transcription errors caught on second pre-flight. Final matrix: 39 available / 20 locked / 16 empty. Smoke (`scripts/test-suggestion-engine-t2.js`): **2690 pass, 0 fail** = 1531 T2 baseline + 6 T3 throws + 75 matrix cells + 39 generation cells (with shape + duration window assertions) + 2 bracket-validation throws + 4 W4 locked/empty safety-net throws + 3 W1 user-aware spot-checks. `/review` grade **A-** — 5 of 6 issues applied (W4 locked/empty throws, W3 `BRACKET_TABLE` exported, W1 `userId` on `getAvailableDurations`, W2 SIGINT/SIGTERM handlers in smoke, S2 `Promise.all` in `generateStateFocus`); architecture extraction recommendation deferred to sprint close. Tech-debt score 3/10. Branch `s12-t3.5` off `s12-t3`, commit `8825ba5`. Spec: `Trackers/S12-T3.5-state-focus-refactor-spec.md`; Amendment: `Trackers/S12-T3.5-AMENDMENT-1-appendix-a-correction.md` (v1.1). |
| 4 | Mobility + Full Body special-case branches | ⏳ Pending | Replaces T2's `NotImplementedError` throws for `mobility` / `full_body` with the practice-type / compound-detection paths per spec §Special Cases. Will branch from `s12-t3.5`. |
