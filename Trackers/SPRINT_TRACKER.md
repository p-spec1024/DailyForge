...
**Progress:** T1 ✅ → T2 ✅ → T3 ✅ → **T3.5 ⏳ (next)** → T4 → T5 → T6 → T7.
...
| 3.5 | State-focus refactor — range-bracket picker + `getAvailableDurations` + `centering/practice/reflection` phase rename | ⏳ Next | **Architecture pivot driven by T3 smoke results — see note below.** Replaces T3's fixed-budget contract with the v2 spec's range-bracket model. Picker exposes 5 brackets: `0–10 / 10–20 / 21–30 / 30–45 / endless`. New `getAvailableDurations(focus_slug, breathwork_level)` helper returns the eligible bracket set per (focus, level), so the UI hides ranges the content genuinely can't fill (e.g. energize beginner has Morning Energizer max=8 → only `0–10` available). Engine output renames `settle / main / integrate` → `centering / practice / reflection` (user-facing terms per Appendix A). Body-focus paths, mobility, full_body, recency, swap-counter, HTTP all unchanged. **T3.5 is a refactor, not a rewrite** — T3's settle/main/integrate logic and DB queries carry forward; the changes are in (a) the duration-allocation table, (b) the new helper, (c) the phase-name mapping, (d) the `endless` mode (no fixed total — uses technique's natural max). Spec: `Trackers/S12-suggestion-engine-spec.md` v2, §State Focus Picker (Range Brackets), §`getAvailableDurations` Contract, Appendix A. Will branch from `s12-t3`. |
| 4 | Mobility + Full Body special-case branches | ⏳ Pending | Replaces T2's `NotImplementedError` throws for `mobility` / `full_body` with the practice-type / compound-detection paths per spec §Special Cases. Will branch from `s12-t3.5` (or `s12-t3` if T3.5 hasn't merged yet). |
