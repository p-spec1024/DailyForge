...
| 3 | Save Workout as Routine + Timer Fix | main | ✅ Shipped | Apr 13 | Apr 13 | SaveRoutineModal (save from header menu or post-workout summary), routines CRUD API + DB schema, My Routines horizontal scroll cards on Strength page (start/delete), timer deferral for empty workouts (00:00 until first exercise added), eager session start for Home→Strength (timer starts immediately), routine_id tracking for last_used. 3 review iterations: fixed undeferTimer guard, resumeSession timer reset, strengthOnly error recovery. |
| 4 | Resume Logic Fix | main | ✅ Shipped | Apr 13 | Apr 13 | **Phase 1 (routing):** startSession accepts opts.type, handleResume routes by session.type. **Phase 2 (state restoration):** /session/start returns logged_sets + exercise metadata for existing sessions, startSession() restores exerciseSets instead of clearing, EmptyWorkoutView populates exercises array from resumed data, timer ticks from original startedAt, rest timer restored from sessionStorage. |

**Progress: 4/4 tickets shipped**
