...
| 2 | Cross_pillar 5-phase orchestrator + home Start (promoted from original T3 per Amendment 1) | ⏳ Planned | Builds the 5-phase orchestrator (bookend_open[breath] → warmup[yoga] → main[strength] → cooldown[yoga] → bookend_close[breath]). Adds `cross_pillar` branch to `SessionLauncher`; wires home `_onStart` to launcher. Phase persistence, transitions, skip/shorten UX. Inherits launcher pattern from T1. Pre-flight depth: HIGH. |
