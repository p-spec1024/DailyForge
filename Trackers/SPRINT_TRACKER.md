...
| 5b | - Backend Endpoints (muscle heatmap, flexibility, recent wins) | ✅ Done | Three endpoints under `/api/body-map/*` (muscle-volumes, flexibility, recent-wins). Built on `lib/data/mock_body_map_data.dart` as the contract. Smoke test at `server/scripts/test-body-map-endpoints.js` (67 checks pass). Branch `s10-t5b`, commits `501714e` (initial), `a79cb29` (review fixes), data cleanup `2026-04-24`. **Mock divergence:** `mockRecentWins` is `List<Map<String,String>>` with only `icon/title/subtitle` — no `type` or `achieved_at` as the original spec assumed; endpoint returns the mock shape. **Data cleanup (Apr 24):** removed 11 dirty yoga rows (empty / prose-only `target_muscles`) + populated 14 with proper muscle tokens. Yoga pose count 269 → 258, region coverage 87.7% → 93.8%. See `server/scripts/{cleanup,populate}-yoga-muscles-2026-04-24.mjs` for the applied migrations. |
