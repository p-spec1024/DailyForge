...
## Sprint 14 — Session Start Handoff (planned)

Dedicated sprint for the session start UX. Each ticket gets its own spec
authored before prompt dispatch.

| Ticket | Scope | Depends on | Size |
|--------|-------|------------|------|
| S14-T1 | SessionLauncher abstraction + home Start wiring + strength startFromList (server endpoint + client provider) + body-focus strength_only end-to-end | — | M |
| S14-T2 | Yoga player engine adapter (phase remap main→peak, holdSeconds conversion, content_id→id rename, defaults for missing fields) + body-focus yoga_only end-to-end | T1 | M |
| S14-T3 | Five-phase orchestrator skeleton — FivePhaseSessionPage, phase state machine, phase indicator UI, pause/resume across phases. Embedded players stubbed. | T1, T2 | L |
| S14-T4 | Player embedding refactor — strength/yoga/breathwork gain embedded mode. Cross-pillar runs end-to-end after this. | T3 | L |
| S14-T5 | State-focus 3-leg chain — leg manager, breathwork completion handlers, silent-timer page for reflection, skip/back leg UX. | T1 | M |
| S14-T6 | Polish + edge cases — multi-phase completion summary, skip/shorten phase UX, mid-session phase preview, recency warning surfacing. | T1–T5 | M |
