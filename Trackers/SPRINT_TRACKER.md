...
| 3 | Yoga adapter + Yoga tab Start (demoted from original T2 per Amendment 1) | ⏳ Planned | Adds yoga branch to `SessionLauncher` + engine→yoga adapter (phase remap main→peak, holdSeconds conversion). New "Today's yoga session" card on `YogaPage` mirroring T1's Strength tab pattern. Inherits T1's launcher pattern + T2's cross-cuts. |
