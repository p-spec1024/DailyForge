...
## 🚀 Before Android Launch (45 items)
...
| 113 | 5-phase sessions split across pillar bars | Analytics completeness | T5c-b ships with `type IN ('strength', '5phase')` treated as strength in weekly chart and minutes this week. Once Sprint 11 ships the 5-phase orchestrator and users run real 5-phase sessions, revisit: (a) give 5phase its own bucket in the weekly chart (4th color/stack segment), or (b) split proportionally across breath/yoga/strength bars based on phase durations. 2-line change per T5c-b build report. Added Apr 24, 2026. |
...
| 118 | Custom workout builders per pillar | Feature | User-built routines inside each pillar page (strength / yoga / breathwork). User picks exercises/poses/techniques, saves the routine, starts it. Distinct from the home page — lives on the individual pillar pages. Each pillar gets its own builder UX tuned to that pillar's primitives (sets × reps for strength, pose sequences for yoga, inhale-hold-exhale patterns for breathwork). Added Apr 24, 2026. |
| 119 | Adaptive home page personalization | UX / ML | Beyond the static home page redesign: home page layout adapts based on user's actual usage pattern. If user does yoga every morning, yoga card rises to the top. If user does strength 5x/week and breathwork 1x/month, strength is more prominent. Reference: Spotify 2026 layout personalization, Google Maps context modes. Requires usage telemetry + simple ranking algorithm. Sprint 12+ scope. Added Apr 24, 2026. |
| 120 | Full session philosophy planning session | Product | DEDICATED PLANNING NEEDED before Sprint 11 T1 ships the 5-phase orchestrator. Scope: (1) physiological / ancient-text justification for the 5-phase mix (breath → yoga warmup → strength → yoga cooldown → breath); (2) cardio integration decision — where/how/how often cardio fits into a full session (Prashob leaning toward including cardio IN full session rather than as a separate pillar); (3) personalization algorithm for generating today's full session for this user; (4) onboarding flow for new users. NOT a code ticket — this is a strategic planning session that produces decisions + sub-tickets. Parked Apr 24, 2026. |
| 121 | Cardio integration in full session | Product | Do NOT build cardio as a 4th pillar or dedicated page. Instead, fold cardio INTO the full 5-phase session. Needs research: where it fits (before/after strength?), how often per week, what counts as cardio for a mind-body app. Discuss in the full session philosophy planning session. Added Apr 24, 2026. |
...
| 99 | Today's session card Start button wires to real workout | Feature completion | Currently the Start button on TodaySessionCard is non-functional (marked TODO(S10-T5c) in code). Wire it to start the 5-phase session. Lives in T5c/Sprint 11 anyway; capturing here for completeness. Added Apr 23, 2026. |
