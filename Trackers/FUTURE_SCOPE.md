...
| 107 | api_service.dart ClientException rewrap | DevEx | T5c-a phone test #1 surfaced this: `_send` and `getList` in `lib/services/api_service.dart` catch `SocketException` / `TimeoutException` / `HttpException` and rewrap them as the `ApiException`-derived types providers expect. The `http` package's `ClientException` (thrown by Android for some network failures) is NOT a subclass of `SocketException`, so it slips through unwrapped — every provider then either swallows it silently or needs a defensive `catch (e)` (`BodyMapProvider` has one as the v1 workaround). Fix: add `on http.ClientException catch (e) → throw NetworkException(e.message)` to both methods. Then drop the defensive catch in BodyMapProvider. **Do before adding the next provider** to prevent the workaround pattern from spreading. Added Apr 24, 2026. |
| 112 | Muscle-mode selected-muscle card still mock | Feature completion | T5c-a shipped with flexibility-mode card reactively updating from real backend data, but muscle-mode card still reads from `lib/data/mock_body_map_data.dart` (per-muscle `lastTrained` / `volumeLabel` / `topExercise` / `setsThisWeek`). Needs a per-muscle detail endpoint (`GET /api/body-map/muscle-detail/:group`) or an extension of the existing muscle-volumes endpoint to return a detail payload alongside the aggregate. T5c-b scope candidate. The `MuscleDetail.fromJson` factory already exists in `lib/models/body_map.dart` so the Flutter side is a service-method + provider-state-slice change only. Added Apr 24, 2026. |
